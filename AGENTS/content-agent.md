...
> **Account:** `media@` | **Tier:** Phase A | **Outbound:** Gated — `doc@` publishes episodes; `media@` handles distribution drafts
...
| Social copy | Draft platform-appropriate posts (LinkedIn, X/Twitter) for `matt@` approval |
...
```
content-agent drafts → brand voice gate → matt@ reviews → matt@ publishes
```
